...
class ANSIRenderer {
  ...
  renderHeader(): void {
    ...
    this.write(
      ANSI.FG_GREEN + ANSI.BOLD + 'Pure ANSI TUI Spike' + ANSI.RESET + '\n\n'
    );
  }

  renderItems(): void {
    const endIndex = Math.min(
      this.viewportStart + this.viewportSize,
      this.items.length
    );
  ...
  renderFooter(): void {
    ...
    this.write(ANSI.ERASE_LINE);
    this.write(
      `Item ${this.selectedIndex + 1} of ${this.items.length} | Use j/k to navigate`
    );
  }
