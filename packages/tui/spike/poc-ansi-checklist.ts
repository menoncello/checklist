...
class ANSIChecklistTUI {
  ...
  private renderHeader(): void {
    ...
    this.write('║' + ' '.repeat(padding) + title + ' '.repeat(this.width - title.length - padding - 2) + '║');
  ...
  private moveSelection(delta: number): void {
    const newIndex = Math.max(0, Math.min(this.state.items.length - 1, 
    ...
    if (newIndex !== this.state.selectedIndex) {
      ...
      } else if (newIndex >= this.state.viewportStart + this.state.viewportSize) {
  ...
  private handleKeyPress(key: Buffer): void {
    ...
    switch (char) {
      ...
      case 'G':
        ...
        this.state.viewportStart = Math.max(0, this.state.items.length - this.state.viewportSize);
        this.render();
        break;
      case ' ':
      case '\r': // Enter
...
function generateDemoData(): ChecklistItem[] {
  return [
    { id: '1', label: 'Setup Development Environment', checked: true, indent: 0 },
    ...
    { id: '8', label: 'Implement viewport scrolling', checked: true, indent: 1 },
