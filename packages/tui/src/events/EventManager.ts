...
export class EventManager {
  ...
  private processEmission(emission: EventEmission): boolean {
    ...
    let handlerCount = 0;
    const subscriptions = this.subscriptions.get(emission.event);

    if (subscriptions) {
      // Convert to array and sort by priority
      const sortedSubscriptions = Array.from(subscriptions).sort(
      ...
      if (subscriptions.size === 0) {
        this.subscriptions.delete(emission.event);
      }
    }
  ...
  private updateEventMetrics(
    ...
    switch (action) {
      ...
      case 'handler_executed':
        metrics.handlerExecutions += count;
        if (duration !== undefined) {
          metrics.totalHandlerTime += duration;
          metrics.averageHandlerTime =
            metrics.totalHandlerTime / metrics.handlerExecutions;
        }
        break;
      case 'error':
        metrics.errorCount++;
  ...
  public validate(): EventManagerValidationResult {
    ...
    // Check for high subscription count
    if (this.getSubscriptionCount() > 1000) {
      warnings.push(`High subscription count: ${this.getSubscriptionCount()}`);
    }
    ...
    // Check for events with no subscriptions
    const eventsWithoutSubscriptions = this.getRegisteredEvents().filter(
      (event) => this.getSubscriptionCount(event) === 0
    );

    if (eventsWithoutSubscriptions.length > 0) {
      warnings.push(
        `${eventsWithoutSubscriptions.length} event(s) with no subscriptions`
      );
    }
    ...
    this.eventMetrics.forEach((metrics, event) => {
      if (metrics.errorCount > 0 && metrics.emissionCount > 0) {
        const errorRate = metrics.errorCount / metrics.emissionCount;
        if (errorRate > 0.1) {
          warnings.push(
            `High error rate for event '${event}': ${(errorRate * 100).toFixed(1)}%`
          );
        }
      }
    });
