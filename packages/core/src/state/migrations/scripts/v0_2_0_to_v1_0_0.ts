...
export const migration_v0_2_0_to_v1_0_0: Migration = {
  ...
  up: (state: unknown) => {
      ...
      enhancedState.activeInstance = {
        ...
        completedSteps: (
          ...
          return {
            ...
            completedAt:
              ((step as Record<string, unknown>).completedAt as
                | string
                | undefined) ?? now,
            result:
              ((step as Record<string, unknown>).result as
                | string
                | undefined) ?? 'success',
            notes:
              ((step as Record<string, unknown>).notes as string | undefined) ??
              '',
          };
        }),
      };
    ...
    if (
      ...
      enhancedState.checklists = s.checklists.map((checklist: unknown) => ({
        ...
        conditions:
          ((checklist as Record<string, unknown>).conditions as
            | string
            | undefined) ?? '',
        commandResults:
          ((checklist as Record<string, unknown>).commandResults as
  ...
  down: (state: unknown) => {
    ...
    if (
      (rest.activeInstance as Record<string, unknown>)?.completedSteps !==
        undefined &&
      Array.isArray(
        (rest.activeInstance as Record<string, unknown>).completedSteps
      )
      ...
      (rest.activeInstance as Record<string, unknown>).completedSteps = (
        (rest.activeInstance as Record<string, unknown>)
          .completedSteps as unknown[]
      ).map((step: unknown) => {
    ...
    if (rest.checklists !== undefined && Array.isArray(rest.checklists)) {
      rest.checklists = (rest.checklists as unknown[]).map(
        (checklist: unknown) => {
          const {
  ...
  validate: (state: unknown): boolean => {
    ...
    if (s.version === null || s.version === undefined || s.version !== '1.0.0')
      return false;
    if (
