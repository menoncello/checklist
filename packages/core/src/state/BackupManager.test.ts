...
describe('BackupManager', () => {
  ...
  describe('Backup Creation', () => {
    ...
    it('should handle backup errors gracefully', async () => {
      ...
      await expect(invalidBackupManager.createBackup(testState)).rejects.toThrow(BackupError);
    });
  });
  ...
  describe('Backup Recovery', () => {
    ...
    it('should handle missing backup gracefully', async () => {
      await expect(backupManager.recoverFromBackup('nonexistent.yaml')).rejects.toThrow(
        RecoveryError
      );
    });
    ...
    it('should handle corrupted backup', async () => {
      ...
      await expect(backupManager.recoverFromBackup('corrupted.yaml.123')).rejects.toThrow(
        RecoveryError
      );
    });
    ...
    it('should throw when no backups available', async () => {
      ...
      await expect(backupManager.recoverFromLatestBackup()).rejects.toThrow(RecoveryError);
    });
  });
