...
// Export CLIApplication class for testing
...
class CLIApplication {
  ...
  async run(): Promise<void> {
    try {
      // Parse arguments using provided argv or Bun.argv for performance
      const parsedArgs = CommandParser.parse(this.argv);
      ...
      // In test environment, handle the error for testing but still throw it
  ...
  private handleGlobalFlags(parsedArgs: ParsedArgs): boolean {
    ...
    if (
      ...
      process.exit(ExitCode.SUCCESS);
      return true; // Return true to indicate global flag was handled
    }
  ...
  private hasDebugFlag(): boolean {
    return this.argv.includes('--debug');
  }
}
...
// Execute main function only when run directly (not when imported)
if (import.meta.main) {
